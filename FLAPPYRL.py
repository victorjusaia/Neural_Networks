...
class FlappyBirdEnv(gym.Env):
    def __init__(self):
        ...
        high=np.array([self.screen_width, self.screen_height, self.screen_height, 10], dtype=np.float32),dtype=np.float32)
...
# Envuelto en un DummyVecEnv para que sea compatible con VecNormalize
env = DummyVecEnv([lambda: FlappyBirdEnv()])

# Normaliza observaciones y recompensas
...
model = PPO(
    ...
    env,
    verbose=1,
    device="cuda",  # Usa la GPU si está disponible
    n_steps=2048,  
    batch_size=64,  
    gae_lambda=0.95, 
    gamma=0.99,  
